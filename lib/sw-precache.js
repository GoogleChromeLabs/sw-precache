...
/**
 * Check if a filePath is within a list of filePaths
 * @param {String} filePath
 * @param {Array} filePathsArray
 * @return {boolean}
 */
function match(filePath, filePathsArray) {
  return filePathsArray.some(function(filePaths) {
    return filePaths.some(function(otherFilePath) {
      return filePath === otherFilePath;
    });
  });
}

function getFilesAndSizesAndHashesForFilePaths(
  includedFilePaths,
  excludedFilePathsArray,
  excludeFilePath) {
  return includedFilePaths.map(function(file) {
    // Return null if we want to exclude this file, and it will be excluded in
    // the subsequent filter().
    if (excludeFilePath === absolutePath(file) ||
      match(file, excludedFilePathsArray)) {
      return null;
    }
...
function generate(params, callback) {
  return new Promise(function(resolve, reject) {
    ...
    var includedFilePathsArray = [];
    var excludedFilePathsArray = [];
    params.staticFileGlobs.forEach(function(globPattern) {
      var toPush = includedFilePathsArray;
      var safePattern = globPattern.replace(path.sep, '/');
      if (safePattern.charAt(0) === '!' && safePattern.charAt(1) !== '(') {
        toPush = excludedFilePathsArray;
        safePattern = safePattern.substring(1);
      }
      toPush.push(glob.sync(safePattern));
    });

    includedFilePathsArray.forEach(function(filePaths) {
      var filesAndSizesAndHashes = getFilesAndSizesAndHashesForFilePaths(
        filePaths, excludedFilePathsArray, params.outputFilePath);
