...
function match(filePath, globs) {
  return globs.some(function(glob) {
    return glob.some(function(globPath) {
      return filePath === globPath;
    });
  });
}

function getFilesAndSizesAndHashesForGlob(glob, excludes, excludeFilePath) {
  return glob.map(function(file) {
    // Return null if we want to exclude this file, and it will be excluded in
    // the subsequent filter().
    if (excludeFilePath === absolutePath(file) || match(file, excludes)) {
      return null;
    }
...
function generate(params, callback) {
  return new Promise(function(resolve, reject) {
    ...
    params.staticFileGlobs.forEach(function(globPattern) {
      ...
      toPush.push(glob.sync(safePattern, {noDir: true})); ;
    });

    includes.forEach(function(glob) {
      var filesAndSizesAndHashes = getFilesAndSizesAndHashesForGlob(
        glob, excludes, params.outputFilePath);
